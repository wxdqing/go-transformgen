...
--target go|csharp|cpp        目标语言
--side requester,responder    生成端类型
--runtime emit|import         runtime 处理方式，Go 默认 emit，C#/C++ 仅支持 emit
--out <dir>                   输出目录
--package <name>              输出包名或 namespace
--cpp-proto-include-prefix <p> C++ 引用 protoc *.pb.h 头文件的 include 前缀
...
C++ target 面向 C++23，message 类复用官方 protoc C++ 产物（`*.pb.h/*.pb.cc`），transformgen 只生成协议绑定：

- `protocol_runtime.hpp`：传输中立抽象（`PacketContext`/`Packet`/`PacketEndpoint`），业务实现 `PacketEndpoint` 对接自己的网络层。
- `protocol_messages.hpp/.cpp`：`enum class EMsgToServerType/EMsgToClientType : std::uint32_t`、`ToMessageId`、`MessageMeta`/`FindMessageMeta`、`CreateMessage(message_id)` 工厂，以及编译期 `MessageId<T>`/`MessageIdOf<T>` 特化。
- `<module>_requester.hpp/.cpp`：模块子命名空间（如 `transform::player`）内的 `Requester`，提供 `SendXxx` / `OnXxxResponse` / `OnXxxNotify`；类名不带模块前缀。
- `<module>_responder.hpp/.cpp`：同一子命名空间内的 `ResponderHandler`（纯虚接口）与 `Responder`（注册回调、解码、回复、`SendXxxNotify`）。

`--package` 指定顶层 namespace（推荐 `transform`），`--side` 控制生成 requester/responder，`--cpp-proto-include-prefix` 控制 `#include "<prefix>/<proto>.pb.h"` 前缀。错误处理沿用 `bool` 返回值（与接入方现有约定一致），不使用 `std::expected`。嵌套 message 暂不支持作为 RPC/notify 消息。
